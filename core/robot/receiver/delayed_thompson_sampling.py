...
class DelayedThompsonSamplingReceiver(GenericReceiver):
    ...
    def __init__(self, args):
        ...
        self.mean          = np.ones([args.n_state])
        self.count         = np.ones([args.n_state])
        self.var           = np.ones([args.n_state])
    ...
    def select_action(self):
        ...
        theta  = np.random.randn(args.n_state) * np.sqrt(self.var) + self.mean
    ...
    def update_sampling_distribution(self):
        ...
        if len(self.n_wait_list) >= args.n_delay:
            ...
            # only update if it is the smallest action
            # that is the one who is matter in this sequence
            if n_wait <= np.mean(self.n_wait_list):
                reward = np.mean(self.reward_list[0])
                # print(f'{n_wait=} {reward=:0.3f}')
                # update sampling distribution
                self.mean[n_wait]  += (reward - self.mean[n_wait]) / self.count[n_wait] ** args.exp_coeff
            # non responsible arm get reduce variance anyway
            # accelerate learning
            else:
                self.count[n_wait] += 1
                self.var[n_wait]    = 1 / self.count[n_wait] ** args.exp_coeff
