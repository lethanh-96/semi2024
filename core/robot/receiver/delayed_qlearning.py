...
class DelayedQLearningReceiver(GenericReceiver):
    ...
    def __init__(self, args):
        ...
        # save old state, action, reward for delayed q-function update
        self.state_list    = []
        self.action_list   = []
        self.reward_list   = []
    ...
    def update_q_table(self, next_state):
        ...
        if len(self.state_list) >= args.n_delay:
            # extract old state, action, average reward
            state     = self.state_list[0]
            action    = self.action_list[0]
            reward    = np.mean(self.reward_list)
            ...
            # pop 1st element from state, action, reward list
        ...
        while 1:
            try:
                if len(self.peak_aoi_list) >= args.n_sample_min:
                    ...
                    # update Q-table
                    self.update_q_table(self.next_state)
                    # update state
